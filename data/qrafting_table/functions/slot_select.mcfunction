...
# クリック判定のアマスタを召喚
execute unless entity @e[tag=QT_Det,distance=..0.001,limit=1] run summon armor_stand ~ ~-0.98 ~ {Tags:[QT_Det],Small:true,NoGravity:true,ShowArms:true,Invisible:false,Pose:{RightArm:[-25f,-90f,0f],LeftArm:[-25f,90f,0f]},HandItems:[{},{id:"stone_button",Count:1b,tag:{display:{Name:'""'}}}]}
...
execute positioned ~ ~-0.98 ~ if entity @e[tag=QT_Det,distance=..0.001,limit=1,x_rotation=1] positioned ~ ~0.98 ~ run function qrafting_table:slot_click
